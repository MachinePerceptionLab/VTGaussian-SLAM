...
class GradSLAMDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        color_interpolation=cv2.INTER_LINEAR,
    ...
    def _preprocess_color(self, color: np.ndarray, interpolation=cv2.INTER_LINEAR):
        ...
        """
        # print('color_interpolation', interpolation)
        color = cv2.resize(
            color,
    ...
    def _preprocess_depth(self, depth: np.ndarray, interpolation=cv2.INTER_NEAREST):
        ...
        """
        # print('depth_interpolation', interpolation)
        depth = cv2.resize(
            depth.astype(float),
            (self.desired_width, self.desired_height),
            interpolation=interpolation,
        ...
        #     depth = cv2.inpaint(depth, (depth == 0).astype(np.uint8), 3, cv2.INPAINT_NS)
    ...
    def __getitem__(self, index):
        ...
        color = self._preprocess_color(color, interpolation=self.color_interpolation)
        ...
        depth = self._preprocess_depth(depth, interpolation=self.depth_interpolation)
        ...
        # only one pixel is retained
        # print(color.shape, depth.shape)
        # color = color[100, 100, :].reshape(1, 1, 3)
        # depth = depth[100, 100, :].reshape(1, 1, 1)
        # print(color, depth)
